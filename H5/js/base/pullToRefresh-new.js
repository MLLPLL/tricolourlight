...
var refresher = {
	...
	scrollIt: function(parameter,pullUpEl, pullUpOffset) {
		eval(parameter.id + "= new iScroll(parameter.id, {useTransition: true,vScrollbar: false,onRefresh: function () {refresher.onRelease(pullUpEl);},onScrollStart:function(){refresher.onScrollStar(this);},onScrollMove: function () {refresher.onScrolling(this,pullUpEl,pullUpOffset);},onScrollEnd: function () {refresher.onPulling(pullUpEl,parameter.pullUpAction);},})");
	...
	onScrollStar:function(e) {
		refresher.info.sy = e.y;
		//console.log("refresher.info.sy",e); 

	},

	onScrolling: function(e,pullUpEl, pullUpOffset) {
		var t=refresher.info.sy-e.y;
		
                 // console.log("t.",refresher.info.sy-e.y); 

            // if(t<p){//下滚
            //     alert('下滚');
            //     // $('.sub-lassify-box').css('display','none');
            //     $('#classify').hide();
            //     $('.sub-classify-box').css('display','block');
            //     $('.sub-classify-box').removeClass('fixedTop108').addClass('fixedTop');
            //     $('.fixedTop-fill').show();
            // }       
            // else{
                //  $('#classify').show().addClass('fixedTop');
                //  $('.sub-classify-box').addClass('fixedTop').removeClass('fixedTop108');
                // alert('shang滚');  
                // if($('#classify').css('display','block')){                   
                //     $('.sub-classify-box').removeClass('fixedTop').addClass('fixedTop108');
                // }else{
                //     $('.sub-classify-box').addClass('fixedTop').removeClass('fixedTop108')
                // }
            // }
            // if(p==0){
            //     $('#classify').removeClass('fixedTop');
            //     $('.sub-classify-box').removeClass('fixedTop108')
		...
		if (0> t) {
			...
			// console.log('上');
                ...
                $('.sub-classify-box').addClass('fixedTop').removeClass('fixedTop108');
                $('.fixedTop-fill').hide();
                if($('#classify-box').css('display','block')){   
		...
		if (-40==refresher.info.sy){
			     $('#classify-box').removeClass('fixedTop');
                $('.sub-classify-box').removeClass('fixedTop108')
		}
	...
	onRelease: function( pullUpEl) {
		...
		if (pullUpEl.className.match('loading')) {
			...
			pullUpEl.style.lineHeight ="28px";
		}
	},
